...
//
// The first 200 samples form the half Hamming portion; the trailing
// 40 samples form the quarter cosine portion (cos(0) at n=200 down to
// cos(2π·39/159) at n=239). Values are round-half-to-even projections
// of v · 32768 onto int16 with saturation at +32767 (n ∈ {199, 200}).
//
// This literal LUT is the production data; its arithmetic equivalence
// to eq. 3 is asserted at every index by the oracle test in
// window_test.go (TestLPAnalysisWindow_OracleAllIndices). Following
// the Phase 1a CosLSP precedent, only the literal table ships in
// production; the floating-point oracle lives test-side.
var lpAnalysisWindow = [240]int16{
