...
func TestApplyTilt_HalfMuAttenuatesPositiveCorrelation(t *testing.T) {
	var pf Postfilter
	var sIn, sOut [subframeLen]int16
	for i := range sIn {
		sIn[i] = 100
	...
	for i := 1; i < subframeLen; i++ {
		if sOut[i] < 49 || sOut[i] > 51 {
			t.Errorf("sOut[%d] = %d, want 50 (±1)", i, sOut[i])
		}
	}
}
...
func TestComputeTiltMu_SinglePoleHalf(t *testing.T) {
	...
	const want = 13068
	if mu != want {
		t.Fatalf("μ from Annex A fixed-point autocorrelation path: want %d, got %d", want, mu)
	}
}
