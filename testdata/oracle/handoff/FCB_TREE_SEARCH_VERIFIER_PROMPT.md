# FCB Tree-Search Verifier Prompt
...
- The pinned input file is `600064` bytes (`300032` little-endian
  signed 16-bit samples) with SHA-256
  `0fceb7702a05d09a9a39d5442c56b827ae0f43eb88a33746ab6f28e7ab5849d5`.
- Frame numbers use 10 ms G.729 frame indexing over the int16 sample
  stream.
- The included frame range is `292..294`, covering both subframes.
...
- Match rows between the template and the `got` file only by
  `field,frame,sub,index`.
- Do not run `G729_WRITE_FCB_TREE_SEARCH_HANDOFF=1` after filling; it
  regenerates the blank template.
...
```sh
G729_COMPARE_FCB_TREE_SEARCH_HANDOFF=1 G729_REQUIRE_COMPLETE_FCB_TREE_SEARCH_HANDOFF=1 G729_REQUIRE_EXACT_FCB_TREE_SEARCH_HANDOFF=1 go test -run TestOracleHandoff_CompareFCBTreeSearchHandoff -count=1 -v
```
