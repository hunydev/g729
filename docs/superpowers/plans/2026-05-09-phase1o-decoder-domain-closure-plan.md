...
- **D-3.tame** — `TestDecode_ITUVectorTameBitExact` (line 475). Smallest non-pathological vector (128 frames). Re-enable first; if PASS, treat as pilot evidence the rest will follow; if FAIL at sample 5..7, confirm gate-17-window only and proceed per D-1 disposition mapping; if FAIL elsewhere, escalate to a measurement sub-task.
  ...
  - [x] **D-3 S-2 H-1 fix attempt 1 of 5** *(this commit)* — REFUTED. Removing the lazy seed (per §4.3 catch-all) shifted the TAME divergence from frame 0 sample 1 to sample 0 instead of resolving it; all 6 ITU vectors still FAIL. Lazy seed restored. Per §8 risk R-1, §A.4.2.4 binding clause "initialized to g_target" stands; sample-0 match pre-fix was non-causal masking. `internal/decoder/phase1o_d3_s2_h1_fix_test.go` preserved as `t.Skip` refutation record. Budget 1/5 consumed; H-2 (`applyAGC` iteration internals) is next surviving rank-1 candidate, but signature (Δ=-2 at sf-0 sample 0 every vector) suggests S-3 should re-rank toward upstream synth→postfilter handoff first.
